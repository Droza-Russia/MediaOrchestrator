...
| 🧠 **Подготовка аудио для ИИ** | Есть отдельный high-level сценарий для ASR/STT: `NormalizeAudioForTranscription(...)` нормализует первую аудиодорожку или выбранный audio stream в **mono / 16 kHz / PCM s16le / WAV** либо в пользовательский target-контракт. |
| ⚠️ **Ошибки** | Тексты исключений и системных сообщений — **на трёх языках: русский, английский, немецкий** (`LocalizationLanguage` / `SetExecutablesPath` / `SetLocalizationLanguage`). Язык по умолчанию при настройке пути — английский. Все библиотечные ошибки сведены к общему базовому типу **`XabeFFmpegException`**, а файловые/потоковые/права доступа разнесены по более точным исключениям. |
...
> 🎙️ **Практический AI-сценарий:** для транскрибации речи можно не собирать `-vn -ac 1 -ar 16000 -c:a pcm_s16le -f wav` вручную, а использовать `FFmpeg.Conversions.FromSnippet.NormalizeAudioForTranscription(...)`.
...
FFmpeg.SetExecutablesPath(
    ...
    language: LocalizationLanguage.English, // или Russian, или German — тексты ошибок на выбранном языке
...
- `buildTransitive`-target, который при restore/build потребителя по умолчанию устанавливает **английскую** документацию и обновляет один итоговый файл `Xabe.FFmpeg.xml`, который читает IDE рядом со сборкой пакета

Идея простая: у потребителя не пачка XML-файлов и не ручной выбор документации, а **один** `Xabe.FFmpeg.xml`, который IDE читает как обычно. По умолчанию используется английский шаблон; при необходимости можно явно выбрать русскую локаль.
